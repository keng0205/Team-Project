<?php
	...
	$query = 'SELECT * from orphanagehome where oHStatus="Active"';
	...
	if(isset($_GET['searchOH'])){
		...
		$query = 'SELECT * from orphanagehome WHERE oHStatus="Active" AND ohName LIKE "%'.$search.'%"';
		$result = mysqli_query($db, $query);
	}
	...
	if(isset($_GET['state'])){
		...
		unset($_GET['searchOH']);
		if($state!="All"){
			$query = "SELECT * from orphanagehome where oHStatus='Active' AND oHState='$state'";
		}
		else{
			$query = 'SELECT * from orphanagehome where oHStatus="Active"';
		}
...
<body onload="setTab()">
	...
	<ol class="breadcrumb">
			...
			<li class="breadcrumb-item">Orphanage home List</li>
		</ol>
		
		<h2>Orphanage Home List</h2>
		<hr>
		...
		<div class="changeToArchiveList">
			<form action="orphanageHomeArchiveList.php">
					<button  class="btn btn-secondary" name="changeOrphanageHomeList" type="submit"><span>View Archive List</span></button>
			</form>
		</div>
		<div class="stateList">
			...
			if($result->num_rows == 0){
				...
				echo "<table class='table'>
						<thead class='thead-inverse'>
							<tr>
								<th>No.</th>";?>
								<th><a href='viewOrphanageHome.php?<?php if(isset($_GET['searchOH'])){echo "searchOH=".$_GET['searchOH']."&";}?>state=<?php echo $state?>&nameSort= <?php echo isset($_GET['nameSort'])?!$_GET['nameSort']:""; ?>'>Name</a></th>
								<th><a href='viewOrphanageHome.php?<?php if(isset($_GET['searchOH'])){echo "searchOH=".$_GET['searchOH']."&";}?>state=<?php echo $state?>&stateSort= <?php echo isset($_GET['stateSort'])?!$_GET['stateSort']:""; ?>'>State</a></th>
				...
				while($row = $result->fetch_assoc()){	
					...
					<?php if(countRight($right,16)) {
					echo "<td><a href=\"archiveOrphanageHome.php?id=".$row['oHID']."\">Archive</a></td>";
					}
					echo "</tr>";
					echo "</tbody>";    
