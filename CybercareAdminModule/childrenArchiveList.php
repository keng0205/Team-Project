<?php
	...
	$query = 'SELECT * from childrenarchive WHERE ohid ="'.$oHID.'"';
	$result = mysqli_query($db, $query);
	$search = "";
	...
	if(isset($_GET['searchCL'])){
		...
		if(is_numeric($search)){
			$query = 'SELECT * from childrenarchive WHERE ohid ="'.$oHID.'" AND cIC LIKE "%'.$search.'%"';
			$result = mysqli_query($db, $query);
		}
		else{
			$query = 'SELECT * from childrenarchive WHERE ohid ="'.$oHID.'" AND cName LIKE "%'.$search.'%"';
			$result = mysqli_query($db, $query);
		}
	}
...
<body onload="setTab()">
	...
	<ol class="breadcrumb">
			...
			<li class="breadcrumb-item">Archived Children List</li>
		</ol>
		<h2>Children List</h2>
		<hr>
		...
		<div class="changeToChildrenList">
			<form action="childrenList.php">
					<button class="btn btn-secondary" name="changeChildrenList" type="submit"><span>View Children List</span></button>
			</form>
		</div>
				...
				if($result->num_rows == 0){
					...
					echo "<table class='table'>
							<thead class='thead-inverse'>
								<tr>
									<th>No.</th>
									<th><a class=\"sortTH\" href=\"childrenArchiveList.php?searchCL=$search&sorting=$sort&field=cIC\">IC(MyKid/ MyCard)</a></th>
									<th><a class=\"sortTH\" href=\"childrenArchiveList.php?searchCL=$search&sorting=$sort&field=cName\">Name</a></th>
									<th>Age</th>
								</tr>
							</thead>";
