<?php
	...
	$query = 'SELECT * from children c , orphanagehome o WHERE c.oHID=o.oHID';
	$result = mysqli_query($db, $query);
	...
	if(isset($_GET['searchCL'])){
		...
		if(is_numeric($search)){
			$query = 'SELECT * from children c , orphanagehome o WHERE c.oHID=o.oHID AND cIC LIKE "%'.$search.'%"';
			$result = mysqli_query($db, $query);
		}
		else{
			$query = 'SELECT * from children c , orphanagehome o WHERE c.oHID=o.oHID AND cName LIKE "%'.$search.'%"';
			$result = mysqli_query($db, $query);
		}
	}
...
<body onload="setTab()">
	...
	<ol class="breadcrumb">
			...
			<li class="breadcrumb-item">Search Children</li>
		</ol>
		...
		<div class="changeToArchiveList">
			<form action="searchChildrenArchiveList.php">
					<button class="btn btn-secondary" name="changeChildrenList" type="submit"><span>Search Archive List</span></button>
			</form>
		</div>
				...
				if($result->num_rows == 0){
					...
					echo "<table class='table'>
							<thead class='thead-inverse'>
								<tr>
									<th>No.</th>
									<th><a class=\"sortTH\" href=\"searchChild.php?searchCL=$search&sorting=$sort&field=cIC\">IC(MyKid/ MyCard)</a></th>
									<th><a class=\"sortTH\" href=\"searchChild.php?searchCL=$search&sorting=$sort&field=cName\">Name</a></th>
