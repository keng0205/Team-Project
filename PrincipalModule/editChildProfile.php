<?php
	...
	$getChildQuery = 'select * from children where cic ="'.$cIC.'"';
	$getChild =  mysqli_query($db, $getChildQuery);
	$childDetails = $getChild->fetch_assoc();
	...
	$selectedYear = $childDetails['year'];
	$selectedSchool = $childDetails['schoolID'];
?>
...
<script>
	...
	var selectedSchool = <?php echo json_encode($selectedSchool) ?>;
	...
	function setLevel(){
		if(selectedYear.indexOf("Standard") == 0){
			var level = document.getElementById("p")
			level.checked = true;
			showPSchoolList();
		}
		else{
			var level = document.getElementById("s")
			level.checked = true;
			showSSchoolList();
		}
	}
	...
	function setSelectedSchool(){
		var schoolList = document.getElementById("schoolListDD");
		
		for(var i = 0; i < schoolList.options.length; i++){
			if(schoolList.options[i].value == selectedSchool){
				document.getElementById(selectedSchool).selected = true;
			}
		}
	}
	
	function setYear(){
		document.getElementById(selectedYear).selected = "true";
	}
	
</script>

<body onload="setTab(); setLevel(); setYear(); setSelectedSchool();">
	...
	<div class="wrapper">
		<ol class="breadcrumb">
			...
			<li class="breadcrumb-item active">Edit Child Profile</li>
		</ol>
		<h2>Edit Child's Detail</h1>
		<hr>
		<form name="form" action="saveChildProfile.php" method="post" onsubmit="return validateChildForm()">
		<table class="formTable">
			<tr>
				<td class="labelTD"><label class="regLabelStyle">IC &#40;MyKid/ MyCard&#41; :</label></td>
				<td class="inputTD"><input class="regInputStyle" type="text" name="cIC" id="cIC" maxlength="12" placeholder="Enter IC number without any symbols. eg: 123456789012" value="<?php echo $childDetails['cIC']?>"></td>
			</tr>
			<tr>
				<td class="labelTD"><label class="regLabelStyle">Name :</label></td>
				<td class="inputTD"><input class="regInputStyle" type="text" name="cName" id="cName" maxlength="50"  value="<?php echo $childDetails['cName']?>"></td>
			</tr>
			<tr>
				<td class="labelTD"><label class="regLabelStyle">Email Address :</label></td>
				<td class="inputTD"><input class="regInputStyle" type="email" name="cEmail" id="cEmail" maxlength="50" placeholder = "Principal's email address will be used if this field is left empty." value="<?php echo $childDetails['cEmail']?>"></td>
			</tr>			
			<tr>
				<td class="labelTD"><label class="regLabelStyle">Academic Level :</label></td>
				<td class = "inputTD eduRadioTd">
					<div>
						<input class="eduRadio" type="radio" name="eduLevel" onchange = "showPSchoolList()" id = "p" value="Primary">Primary School</input>
					</div>
					<div>
						<input class="eduRadio" type="radio" name="eduLevel" onchange = "showSSchoolList()" id = "s" value="Secondary">Secondary School</input>
					</div>
				</td>
			</tr>			
			<tr>
		...
		<hr>
		<div id="regNextDiv">
			<input type='hidden' name='selectedIC' value='<?php echo "$cIC";?>'/> 
			<button class="btn btn-secondary" id= "regChildBtn" name="regChildBtn" type="submit"><span>Save</span></button>
		</div>
		</form>
	</div>
</body>
