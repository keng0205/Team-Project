<?php 
	...
	if(isset($_POST['selectedIC'])){
		$cIC = $_POST['selectedIC'];
	}
	else{
	...
	if(isset($_POST['cIC'])){
		if($_POST['cIC'] != $cIC){
			...
			if($result->num_rows > 0){
				...
				echo '<script type="text/javascript">alert("This child is already registered under :\n'.$oHName.'");
					window.location="editChildProfile.php";</script>';	
			} 
		}
		$cIC = $_POST['cIC'];
		$cName = $_POST['cName'];
		...
		if($db->query($query)){
			mysqli_close($db);
			echo '<script type="text/javascript">alert("Child profile successfully updated.");
			window.location="childInfoProfile.php?id='.$cIC.'";</script>';			
		}
		
	}else{
		header('Location: childInfoProfile.php');
	}
